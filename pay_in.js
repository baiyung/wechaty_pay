...
bot
    ...
    .on('message', async function(message) {
		...
		if(/hello/.test(content)|| /^你好$/.test(content)){
			message.say("你好，我是本次NodeJS技术学习交流群的机器人Nancy，请输入暗号【nodejs】，我将拉你进入会员群")
...
function feeToRoom(contact,room){
    ...
    if (room.has(contact)) {//如果群里已经有该用户，则回复无需再支付
		...
		let id = contact.id
		let tmplink = 'http://10.204.41.41:3000/payment.html?user_id=' + id
			...
			connection.query(sql,selSqlParams, function(err, rows){
				...
				if(rows == 0){//用户还未支付，将支付链接发给用户
					...
					contact.say(tmplink)
						...
						contact.say("Nancy检测到你已经成功付款，正准备将您拉入群聊，稍安勿躁哦~")
